<?php
class HomeModel {
  private $db;

  public function __construct() {
    $this->db = new Connect();
  }

  public function getProductsNew($limit = 8, $page = 1, $keySearch = "")
  {
    $offset = ($page - 1) * $limit;
    $sql = "select sp.id, sp.ten_san_pham, sp.gia_ban, sp.gia_khuyen_mai, sp.anh_dai_dien, dm.ten_danh_muc, dm.id as dmid
      from san_phams sp
      join danh_mucs dm on sp.danh_muc_id = dm.id ";
    if ($keySearch !== '') {
      $sql.= " where sp.ten_san_pham LIKE '%".$keySearch."%'";
    }
    $sql.= "ORDER BY sp.id DESC LIMIT $offset, $limit";
    return $this->db->query($sql);
  }

  public function getProductsPopular($limit = 8, $page = 1, $keySearch = "")
  {
    $offset = ($page - 1) * $limit;
    $sql = "select sp.id, sp.ten_san_pham, sp.gia_ban, sp.gia_khuyen_mai, sp.anh_dai_dien, dm.ten_danh_muc, dm.id as dmid, avg(dg.diem_danh_gia) AS trung_binh_diem
      from san_phams sp
      join danh_mucs dm on sp.danh_muc_id = dm.id
      left join danh_gias dg on sp.id = dg.san_pham_id ";
    if ($keySearch !== '') {
      $sql.= " where sp.ten_san_pham LIKE '%".$keySearch."%'";
    }
    $sql.= "group by sp.id, sp.ten_san_pham order by trung_binh_diem DESC LIMIT $offset, $limit";
    return $this->db->query($sql);
  }

  public function getImageSecond($id) {
    $sql = "select ha.duong_dan from san_phams sp join hinh_anhs ha on sp.id = ha.san_pham_id where sp.id = ? limit 1";
    return $this->db->queryOne($sql, $id);
  }

  public function getImageBanner() {
    $sql = "select  * from banners b  where 1";
    return $this->db->query($sql);
  }

  public function getTotalRateAndCount($id) {
    $sql = "SELECT
      AVG(dg.diem_danh_gia) AS trung_binh_diem,
      COUNT(dg.diem_danh_gia) AS tong_danh_gia
      FROM san_phams sp
      JOIN danh_gias dg ON sp.id = dg.san_pham_id
      WHERE sp.id = ?";
    return $this->db->queryOne($sql, $id);
  }
}
